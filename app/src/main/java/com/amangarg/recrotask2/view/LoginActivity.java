...
import com.amangarg.recrotask2.models.UserModel;
...
public class LoginActivity extends AppCompatActivity implements postResponseInterface {
        ...
        btnLogin.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    ...
                    loginJsonObject.put("loginmode", 0);
                } catch (JSONException e) {
                    e.printStackTrace();
                }

                PostRequest postRequest = new PostRequest(BaseConfiguration.LOGIN_URL, loginJsonObject.toString());
                postRequest.setResponseInterface(LoginActivity.this);
...
//                        });
    ...
    @Override
    public void onResponseReceived(String response, String URL_NAME, Integer responseCode) {
        if (responseCode == 200) {
            // Proceed with Login and Launch an intent to the mains screen
            // Save the token in DB
            Log.e("response of login API", response);
            String token = null;
            try {
                JSONObject loginJSONResponse = new JSONObject(response);
                UserModel user = new UserModel();
                token = loginJSONResponse.getString("token");

            } catch (JSONException e) {
                e.printStackTrace();
            }

            SharedPreferences sp = getSharedPreferences(BaseConfiguration.SHARED_PREF_NAME, MODE_PRIVATE);
            ...
            // IGNORE ALL ERRORS, SHOW: SOMETHING WENT WRONG
        }
    }
}